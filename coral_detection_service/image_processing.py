...
def compute(self, image, detection_parameters={}, detect='', **kwargs):
    try:
        # Process PIL Image here
        image = image if isinstance(image, Image.Image) else Image.open(input).convert('RGB')
        objects_by_label = coral_object_detection(image, model=kwargs['model'], label=kwargs['labels'], interpreter=kwargs.get('interpreter', None), **detection_parameters)
        if detect:
        ...
        predictions = None
    return self._format_results_dict(predictions)
